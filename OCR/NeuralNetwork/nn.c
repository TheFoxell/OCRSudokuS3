...
//Train the Network with MNIST database
...
//Test and show accuracy of the Network with MNIST database
...
//Find the digit of the input image
uint8_t findDigit(Network *network, char *path){
...
//Apply FeedForward function between 2 layers
static void feedForward(Network* network, Image* img){
...
//Return the most probable digit
static uint8_t getClassification(Layer* layer){
    double maxOutput = 0;
    int maxIndex = 0;
